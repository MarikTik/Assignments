// I apologize in advance for my inability to keep the implementation clean
// The run is basically just a bunch of if checks and attempts of interpretation 
// under specific sets of circumstances, since the sample code provided
// didn't display any sort of complex arithmetic, I take it that I don't have to implement 
// it.
     ...
     namespace __details{
          struct custom_string_hash{
               size_t operator()(const std::string &str) const{
                    size_t hash = 0;
                    for (auto c : str) hash += static_cast<size_t>(c);
                    return hash;
               }
          };
     }
     template<typename Value, typename KeyEqual = std::equal_to<std::string>, typename Allocator = std::allocator<std::string>>
     using named_table = hash_table<std::string, Value, __details::custom_string_hash, KeyEqual, Allocator>;
}
#endif

ds::named_table<std::function<long(long, long)>> operators{
     {"+", [](long a, long b) noexcept{ return a + b; }},
     {"-", [](long a, long b) noexcept{ return a - b; }},
     {"*", [](long a, long b) noexcept{ return a * b; }},
     {"/", [](long a, long b) noexcept{ return a / b; }},
     {"%", [](long a, long b) noexcept{ return a % b; }},
     {"^", [](long a, long b) noexcept{ return std::pow(a, b); }}
};
...
class scope{
     public:
          ...
          void assign(const std::string &name, long value){
               if (_variables.contains(name)) _variables.at(name) = value;
               else if (_parent) _parent->assign(name, value);
               else throw std::runtime_error("variable not found");
          }
          void define(const std::string &name, long value){
               _variables.insert(name, value);
          }
          std::variant<long, std::string> get(const std::string &name){
               if (_variables.contains(name)) return _variables.at(name);
               if (_parent) return _parent->get(name);
               return "UNDEFINED";
          }
          void increment(const std::string &name){
               if (_variables.contains(name)) ++_variables.at(name);
               else if (_parent) _parent->increment(name);
               else throw std::runtime_error(name + " not found");
          }
          void decrement(const std::string &name){
               if (_variables.contains(name)) --_variables.at(name);
               else if (_parent) _parent->decrement(name);
               else throw std::runtime_error(name + " not found");
          }
          bool contains(const std::string &name){
               if (_variables.contains(name)) return true;
               if (_parent) return _parent->contains(name);
               return false;
          }
     private:
          std::shared_ptr<scope> _parent;
          ds::named_table<long> _variables;
};

class borg_interpreter{
     public:
     borg_interpreter() 
          :  _current_scope(std::make_shared<scope>(nullptr))
     {
          _scopes.push(_current_scope);
     }
     ...
     void run(std::istream &is){
          ...
          while(std::getline(is, line)) {
               if (functinon_name != ""){
                    _functions.insert(functinon_name, parse_function_body(is));
                    functinon_name = "";
               }
               interpret_line(line);
          }
     ...
     private:
          ...
          void interpret_line(const std::string &line){
               ...
               while(iss >> token){
                    if (token == "COM"){
                         ...
                         long value;
                         char assign_op;
                         iss >> name >> assign_op >> value;
                         if (assign_op == '=') _current_scope->define(name, value); 
                    }
                    else if (token == "PRINT"){
                         std::string name;
                         std::string binary_operator;
                         iss >> name;
                         if (iss >> binary_operator){
                              std::string rhs;
                              if (not (iss >> rhs)) throw std::runtime_error("expected another token after operator");
                              long result = process_binary_operator(name, binary_operator, rhs);
                              std::cout << name << " " << binary_operator << " " << rhs << " is " << result << std::endl;
                         }
                         else{
                              char *end = nullptr;
                              long val = std::strtol(name.c_str(), &end, 10);
                              if (*end != 0) 
                              {    
                                   auto variable = _current_scope->get(name);
                                   std::cout << name << " is ";
                                   if (std::holds_alternative<std::string>(variable)) {
                                        std::cout << std::get<std::string>(variable) << std::endl;
                                   }
                                   else if (std::holds_alternative<long>(variable)){
                                        std::cout << std::get<long>(variable) << std::endl;
                                   }
                              }
                         ...
                         if (_functions.contains(name)){
                              std::istringstream function_body(_functions.at(name));
                              run(function_body);
                         }
                         else{
                              std::string op;
                              ...
                              if (op == "="){
                                   long rhs;
                                   if (not (iss >> rhs)) throw std::runtime_error("expected another token after operator");
                                   _current_scope->assign(name, rhs);
                              }
                              else if (op == "++"){
                                   _current_scope->increment(name);
                              }
                              else if (op == "--"){
                                   _current_scope->decrement(name);
                              }
                         }
          ...
          void init_new_scope(){
               if (not _current_scope){
                    ...
                    auto new_scope = std::make_shared<scope>(_current_scope);
                    _current_scope = new_scope;
               }
               _scopes.push(_current_scope);
          }
          void finish_scope(){
               if (_scopes.empty()) throw std::runtime_error("no scope to finish");
               _scopes.pop();
               if (not _scopes.empty()) _current_scope = _scopes.peek();
               else _current_scope = std::make_shared<scope>(nullptr);
          }
          long process_binary_operator(const std::string &lhs, const std::string &op, const std::string &rhs){
               char *end;
               long val1 = std::strtol(lhs.c_str(), &end, 10);
               if (*end != 0) 
                    val1 = std::get<long>(_current_scope->get(lhs));

              
               long val2 = std::strtol(rhs.c_str(), &end, 10);
               if (*end != 0)
                    val2 = std::get<long>(_current_scope->get(rhs));
               
               return operators.at(op)(val1, val2);
          }
          std::string parse_function_body(std::istream &is){
               size_t nesting = 0;
               std::string line;
               std::string body;
               while(std::getline(is, line)){
          ...
          std::string functinon_name = "";
          ds::named_table<std::string> _functions;
          ds::stack<std::shared_ptr<scope>> _scopes;

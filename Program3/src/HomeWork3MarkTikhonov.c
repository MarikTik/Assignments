...
void print_total(worker_info* workers, unsigned amount) {
	double total = 0;
	for (unsigned i = 0; i < amount; i++) {
		total += workers[i].payment.gross_pay;
	}

	printf("\nTotal Paid to all employees = %.2lf$", total);
}
int main(int argc, char* argv[]) {
	...
	print_total(workers, iterations);
...
//HomeWork3 - Notice that the function set_payment_info calculates all the payment types
// (gross, overtime, base, tax, net) at one go. I think it is more logical to do it like this,
// and not calculate each as a seperate function. If you see it as wrong than comment on my work
// and I will re-do it.
// 
// 
// 
